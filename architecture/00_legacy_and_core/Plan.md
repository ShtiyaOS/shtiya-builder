...
## 4. UI Assembly Line (Hackathon: Fortified Enterprise Fleet)

### `src/app/(apps)` Directory Tree

The route group structure nests three role-scoped groups inside the existing `(apps)` catch-all. Each group gets its own server-component layout that re-enforces role gating independent of the edge middleware.
...
│   └── callback/route.ts
├── (apps)/
│   ├── layout.tsx                        # Root apps layout — mounts AppShell with session
│   ├── (admin)/                          # Role group: admin only
│   │   ├── layout.tsx                    # Server layout — redirects if role ≠ admin
│   │   └── office/page.tsx              # Agent Observability Command Center
│   ├── (owner)/                          # Role group: owner, investor, admin
│   │   ├── layout.tsx                    # Server layout — redirects if role not in allowlist
│   │   ├── owner/
│   │   │   ├── page.tsx                 # GIS Block Assembly Map
│   │   │   └── deal-room/page.tsx       # Pulse Realtime Deal-Room Feed
│   │   ├── acquisition/page.tsx
│   │   ├── capital/page.tsx
│   │   └── escrow/page.tsx
│   ├── (contractor)/                     # Role group: contractor, architect, admin
│   │   ├── layout.tsx                    # Server layout — redirects if role not in allowlist
│   │   ├── contractor/
│   │   │   ├── page.tsx
│   │   │   └── vision/page.tsx          # Vision AI Sandbox
│   │   ├── design/page.tsx
│   │   └── escrow/page.tsx
│   ├── legal/page.tsx                   # Attorney — no dedicated group (middleware-only)
│   ├── property/page.tsx
│   └── marketing/page.tsx
├── unauthorized/page.tsx                # 403 landing page (role mismatch)
└── api/
    ├── milestone-upload/route.ts        # Hardened Vision upload
    └── ...
```

### Zero-Trust Layout Strategy

Role enforcement operates in two independent layers. A request must clear **both** to reach any data-fetching code:

**Layer 1 — Edge Middleware** (`src/middleware.ts`)
Runs on every `/(apps)/*` request before any rendering occurs. Reads the Supabase JWT, resolves `users.role` from the database, checks `ROLE_APP_MAP`, and issues a redirect to `/unauthorized` on failure. This layer has no HTML output — it is a pure gate.

**Layer 2 — Server Component Layout** (`src/app/(apps)/(group)/layout.tsx`)
Runs after the middleware passes. Each role-group layout independently calls `createServerClient()`, re-fetches the session and role, and calls `redirect('/unauthorized')` if the role is not in the layout's hardcoded allowlist. This layer exists because middleware can be bypassed via direct RSC fetch or if the middleware matcher pattern has a gap. The layout is the final structural enforcement before any DB query fires.

```
Request → Edge Middleware (layer 1) → Server Layout (layer 2) → Page (data fetch + render)
               ↓ fail                        ↓ fail
          /unauthorized               /unauthorized
```

Neither layer trusts the other. The page itself never re-checks the role — role checking is exclusively structural (layout), not conditional logic scattered across UI components.

### Agentic UI Panel Map

| Agent | Table(s) Surfaced | UI Panel | Role Scope |
|-------|-------------------|----------|------------|
| Watchdog | `violations` | `ViolationFeed` in Command Center | admin |
| Vision | `vision_inspections` | `VisionQueue` (admin) + `InspectionResult` (contractor) | admin + contractor |
| Pulse | `deal_room_events` | `PulseFeed` (admin) + `DealRoomFeed` (owner/investor) | admin + owner/investor |
| Match | leads → `properties.embedding` | `MatchLog` in Command Center | admin |
| Sentinel | `compliance_checks` | `SentinelPanel` in Command Center | admin |

---

## 5. Investor ↔ Owner Connection Model

### Design Decision
`owner` and `investor` are separate, non-overlapping personas. An investor may discover many owners but gains direct communication with a specific owner only after that owner explicitly accepts a connection request. `ROLE_APP_MAP` is intentionally kept separate — investors never access `/owner` and owners never access `/acquisition`.

### New Table — `investor_owner_connections` (migration 0010)

| Column | Type | Notes |
|--------|------|-------|
| `id` | uuid PK | |
| `investor_id` | uuid → users | FK, on delete cascade |
| `owner_id` | uuid → users | FK, on delete cascade |
| `status` | `connection_status` enum | `requested` → `accepted` / `declined` / `withdrawn` |
| `message` | text | Optional intro from investor |
| `requested_at` | timestamptz | |
| `resolved_at` | timestamptz | Set when owner acts |

Unique constraint: `(investor_id, owner_id)` — one record per pair.

### Connection Status Machine

```
[none] → requested → accepted   (full data + deal-room access unlocks)
                  → declined
         requested → withdrawn  (investor cancels before owner acts)
```
...
| Policy | Who | What |
|--------|-----|------|
| `connections_investor_select` | investor (own rows) + admin | SELECT outbound requests |
| `connections_owner_select` | owner (own rows) + admin | SELECT incoming requests |
| `connections_investor_insert` | investor only | INSERT new request (own `investor_id`) |
| `connections_update` | investor (withdraw own pending) + owner (accept/decline incoming) + admin | UPDATE status — WITH CHECK enforces valid transitions per side |
| `properties_investor_discovery` | investor | SELECT properties (public fields only — column restriction enforced at query level, not RLS) |

### API Routes

| Route | Method | Caller | Action |
|-------|--------|--------|--------|
| `/api/connections` | POST | investor | Send connection request |
| `/api/connections` | PATCH | investor or owner | Accept / decline / withdraw |
| `/api/connections/discovery` | GET | investor | Property discovery feed (public fields only: address, BBL) |

### UI

- **`/acquisition` (investor)** — Owner Discovery feed: property cards with address + BBL + `ConnectButton`. Connect button expands to an optional message field before sending. Pending requests show a "Withdraw" affordance. Accepted shows a green "Connected" badge. Declined shows a grey "Declined" badge.
- **`/owner` (owner)** — Block Assembly Map (2/3 width) + `ConnectionInbox` panel (1/3 width). Inbox shows pending requests with Accept / Decline buttons and resolved requests below. Investor name, email, message, and relative timestamp shown per request.

### What Did Not Change
- `ROLE_APP_MAP` — investor stays out of `/owner`; owner stays out of `/acquisition`
- `block_committee_members` — owner-to-owner block assembly; unrelated to investor connections
- All existing RLS policies — migration 0010 only adds new ones

---

## 6. Bloomberg Terminal Interface (Phase 7)

### Architectural Mandate

The frontend migrates from a page-by-page routing model with a top-navigation `AppShell` to a persistent three-pane "Bloomberg Terminal" interface. The master layout mounted at `src/app/(apps)/layout.tsx` never remounts during navigation — the Left and Right Panes remain alive while the Center Pane streams new page content via the Next.js `{children}` injection point.

### CSS Grid Topology — `grid-cols-[250px_1fr_350px]`

The shell enforces three fixed column tracks:

| Pane | Track | Tailwind Classes | Rendering Strategy |
|------|-------|------------------|--------------------|
| Left — Switcher | `250px` | `border-r border-slate-800 bg-slate-900/50 flex flex-col min-h-0` | **React Server Component** — zero hydration cost |
| Center — Arena | `1fr` | `min-h-0 min-w-0 overflow-y-auto bg-slate-950` | **Mixed** — each micro-app page decides |
| Right — Co-Pilot | `350px` | `border-l border-slate-800 bg-slate-900/50 flex flex-col min-h-0` | **Client Component** — streaming AI SDK |

**Grid blowout mitigation:** The Center Pane receives `min-h-0 min-w-0` to force the CSS Grid algorithm to respect the `1fr` fractional constraint rather than the intrinsic height of deeply nested data tables. Without these constraints, a heavily populated ledger page would expand the center track and compress the sidebars.

The outer container is locked to the viewport: `grid h-screen w-full overflow-hidden` — full-page document scrolling is intentionally disabled. Each pane manages its own internal scroll independently.

### Zero-Trust Navigation — Left Pane Server Component

The Left Pane (`src/components/layout/LeftSidebar.tsx`) is a strict React Server Component. It receives `currentPath` and the user session as props from the layout. It calls `getAllowedApps(role)` and renders **only** the nav items the user's role permits — inaccessible routes are stripped from the HTML before any byte reaches the client. There is no client-side conditional rendering and no hidden DOM elements. This is the "fail closed" pattern: if `getAllowedApps` returns an empty array, the nav renders nothing.

### Active Link State — `x-next-pathname` Header Injection

Determining the active route traditionally requires `usePathname()`, which forces a `'use client'` boundary and defeats the RSC mandate for the Left Pane. The solution:

1. `src/middleware.ts` intercepts every matched request and injects the pathname into the forwarded request headers as `x-next-pathname`
2. The layout reads `headers().get('x-next-pathname')` and passes it as a prop to `LeftSidebar`
3. The Left Pane computes active state with `currentPath.startsWith(href)` — purely on the server, zero JS shipped to the browser for this logic

### Zustand Context Registry — Solving the Stale Closure Anomaly

The Right Pane Co-Pilot must know what the user is looking at in the Center Pane. Standard approaches fail:

| Approach | Why It Fails |
|----------|-------------|
| Pass context to `useChat` hook config | Captured once at layout mount — stale for all subsequent navigations |
| Prop drilling from layout | Layout is a Server Component; Right Pane is a Client Component — no shared prop channel |
| React Context | Triggers layout-wide re-renders on every navigation |

**The solution — Context Registry pattern:**

- `src/store/terminalStore.ts` exports a Zustand store: `{ context: string; setContext: (ctx: string) => void }`
- Each micro-app page calls `setContext('Viewing: Escrow Contract #899, Status: Pending')` in a `useLayoutEffect` on mount
- The Right Pane reads `useTerminalStore.getState().context` inside the `prepareSendMessagesRequest` callback — this fires on every outgoing AI request, not at hook initialization, guaranteeing the freshest context is always sent

```
User navigates: /capital → /escrow
  ↓
capital/page.tsx useLayoutEffect → setContext("Viewing: Draw Approval Queue, 3 pending")
  ↓                                                        ↓
Right Pane sends message ← prepareSendMessagesRequest reads context from Zustand
  ↓
/api/copilot receives { messages, context: "Viewing: Draw Approval Queue, 3 pending" }
  ↓
Gemini 2.5 Pro answers with full awareness of the user's current screen
```

### Agentic Co-Pilot API Route

`src/app/api/copilot/route.ts` uses the Vercel AI SDK (`streamText` + `@ai-sdk/google`) to stream Gemini 2.5 Pro responses. The route:
1. Authenticates via Supabase session — returns 401 if unauthenticated
2. Parses `{ messages, context }` from the request body
3. Builds a dynamic system prompt embedding the context string
4. Streams the response via `result.toDataStreamResponse()`
